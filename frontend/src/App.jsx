...
/* ─── NAVIGATOR ──────────────────────────────────────────────────────────────── */
// [CHANGE 1 & 2] Added `selectedArticle` prop. When present, shows a highlighted
// "selected article" banner at the top and includes it in all AI operations.
// Font sizes bumped for headings/titles throughout this tab.
function NavigatorTab({ articles, bookmarks, selectedArticle }) {
  ...
  // Merge selected article at top of list for AI operations
  const effectiveArticles = selectedArticle
    ? [selectedArticle, ...articles.filter((a) => a.url !== selectedArticle.url)]
    : articles;

  const genBriefing = async () => {
    setLoadB(true);
    try { const r = await summarizeNews(effectiveArticles); setBriefing(r.data.summary); } finally { setLoadB(false); }
  };
  ...
  const ask = async (q) => {
    ...
    setLoadQ(true);
    try {
      const r = await askAI(txt, effectiveArticles);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* [CHANGE 1] Selected article highlight banner */}
      {selectedArticle && (
        <div className="fade-in" style={{
          background: `linear-gradient(135deg, ${DS.accent}12, ${DS.accent}06)`,
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 10, color: DS.accent, letterSpacing: ".12em", fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>
              ◆ Selected Article
            </div>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              style={{ fontSize: 17, fontWeight: 600, color: DS.textPrimary, lineHeight: 1.45, display: "block", marginBottom: 6 }}
            >
              {selectedArticle.title}
            </a>
            {selectedArticle.description && (
              <p style={{ fontSize: 13, color: DS.textSecondary, lineHeight: 1.6 }}>{selectedArticle.description}</p>
            )}
          </div>
          {selectedArticle.urlToImage && (
            <img
              ...
              alt=""
              style={{ width: 90, height: 60, objectFit: "cover", borderRadius: 8, flexShrink: 0 }}
            />
          )}
        </div>
      )}

      {/* [CHANGE 2] Larger heading sizes */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <h2 style={{ fontFamily: DS.fontDisplay, fontSize: 28, fontWeight: 700, letterSpacing: "-.01em" }}>
            Intelligence Briefing
          </h2>
          <p style={{ color: DS.textSecondary, fontSize: 14, marginTop: 5 }}>
            {effectiveArticles.length} articles synthesized into one explorable document
          </p>
        </div>
        <Btn onClick={genBriefing} disabled={loadB || !effectiveArticles.length}>
          {loadB ? <><Spinner /> Generating…</> : "Generate Briefing"}
        </Btn>
      </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {effectiveArticles.map((a, i) => (
              /* [CHANGE 3] Polished source article card with hover highlight */
              <div
                key={i}
                style={{
                  background: DS.surface,
                  border: `1px solid ${selectedArticle?.url === a.url ? DS.accent + "50" : DS.border}`,
                  borderRadius: 12,
                  padding: "14px 18px",
                  display: "flex",
              ...
              >
                <div style={{ flex: 1, marginRight: 12 }}>
                  {selectedArticle?.url === a.url && (
                    <span style={{ fontSize: 10, color: DS.accent, fontWeight: 700, letterSpacing: ".08em", display: "block", marginBottom: 4 }}>
                      ◆ SELECTED
                    </span>
                  )}
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    style={{ fontSize: 14, color: DS.textPrimary, lineHeight: 1.5 }}
                  >
                    {a.title}
                  </a>
                </div>
...
if (!document.getElementById("lp-style")) {
  ...
  s.textContent = `
    ...
    /* [CHANGE 3] Polished article card hover for landing page */
    .lp-article-card { transition: border-color .2s, transform .2s, box-shadow .2s !important; }
    .lp-article-card:hover { border-color: rgba(232,160,52,.35) !important; transform: translateY(-2px); box-shadow: 0 8px 32px rgba(0,0,0,.4) !important; }
  `;
  document.head.appendChild(s);
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   [CHANGE 1] onEnter now accepts an optional article param.
   [CHANGE 2] Article titles bumped to 20px, descriptions to 14px.
   [CHANGE 3] Article cards get lp-article-card class for hover polish.
═══════════════════════════════════════════════════════════════════════════ */
  ...
  return (
    <div style={{
      ...
      <section id="lp-daily" style={{ padding: "40px 24px 20px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "2fr 1fr", gap: 28 }}>
          <div>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, gap: 20 }}>
              <div>
                <div style={{ fontSize: 10, color: "#e8a034", letterSpacing: ".18em", fontWeight: 700, marginBottom: 8 }}>
                  LIVE · {topArticles.length} ARTICLES ANALYSED TODAY
                </div>
                {/* [CHANGE 2] Larger heading */}
                <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 34, color: "#f0ede8", margin: 0, lineHeight: 1.15 }}>
                  India's business news, <span style={{ color: "#e8a034" }}>intelligently</span> understood
                </h2>
                <p style={{ color: "#8a8795", marginTop: 10, maxWidth: 680, fontSize: 15, lineHeight: 1.7 }}>
                  ET AI Newsroom layers generative AI directly onto today's headlines — giving you personalised briefings,
                  story arcs, voice-first Q&amp;A, and vernacular translation. Not a chatbot. A newsroom co-pilot.
                </p>
              </div>
              <div style={{ display: "flex", gap: 8, flexShrink: 0, paddingTop: 4 }}>
                <button onClick={() => onEnter()} className="lp-cta-btn" style={{ padding: "12px 20px", borderRadius: 28, background: "linear-gradient(135deg,#e8a034,#f0b44a)", color: "#000", fontWeight: 700, border: "none", fontSize: 13 }}>
                  Open AI Newsroom
                </button>
              </div>
            </div>

            {/* [CHANGE 1 + 2 + 3] Article cards: clickable to open newsroom with that article, larger title, hover polish */}
            <div style={{ display: "grid", gap: 14 }}>
              {topArticles.map((a, i) => (
                ...
                >
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: "#e8a034", marginBottom: 8, fontWeight: 600, letterSpacing: ".04em" }}>
                      {a.source?.name || "News"}
                    </div>
                    {/* [CHANGE 2] Title bumped to 20px */}
                    <div style={{ fontSize: 20, fontWeight: 700, color: "#f0ede8", lineHeight: 1.4, marginBottom: 8 }}>
                      {a.title}
                    </div>
                    {/* [CHANGE 2] Description bumped to 14px */}
                    <p style={{ color: "#8a8795", fontSize: 14, lineHeight: 1.65, marginBottom: 12 }}>{a.description}</p>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      <span style={{ fontSize: 11, color: "#4a4855" }}>
                        {new Date(a.publishedAt).toLocaleString("en-IN", { hour: "2-digit", minute: "2-digit" })}
                      </span>
                      {/* [CHANGE 1] "Open in Newsroom" uses onEnter(article) */}
                      <button
                        onClick={(e) => { e.stopPropagation(); onEnter(a); }}
                        style={{ fontSize: 12, padding: "6px 12px", borderRadius: 8, background: "#e8a034", border: "none", color: "#000", fontWeight: 600, cursor: "pointer" }}
                      >
                        Open in Newsroom →
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); onEnter(a); }}
                        style={{ fontSize: 12, padding: "6px 10px", borderRadius: 8, border: "1px solid #1e1e2a", background: "transparent", color: "#e8a034", cursor: "pointer" }}
                      >
                        Build story arc
                      </button>
                    </div>
                  ...
                  {a.urlToImage && (
                    <div style={{ width: 130, height: 88, borderRadius: 10, flexShrink: 0, overflow: "hidden" }}>
          ...
          <aside>
            <div style={{ position: "sticky", top: 24, display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ background: "#0d0d14", border: "1px solid #1e1e2a", borderRadius: 14, padding: 20 }}>
                <div style={{ fontSize: 11, color: "#e8a034", marginBottom: 10, fontWeight: 600, letterSpacing: ".06em" }}>ET AI NEWSROOM</div>
                {/* [CHANGE 2] Sidebar heading bumped */}
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, margin: 0, color: "#f0ede8", lineHeight: 1.3, marginBottom: 10 }}>
                  Your personal news intelligence layer
                </h3>
                <p style={{ color: "#8a8795", fontSize: 13, lineHeight: 1.65, marginBottom: 12 }}>
                  Search any topic. Get briefings, ask follow-ups, translate to your language, and see the full story arc — in seconds.
                </p>
                <ul style={{ color: "#8a8795", paddingLeft: 18, display: "flex", flexDirection: "column", gap: 6 }}>
                  {["Voice-first Q&A on any article", "Personalised for 12 reader profiles", "12 Indian language translations", "AI story arc with timeline & players"].map((item, i) => (
                    <li key={i} style={{ fontSize: 13, lineHeight: 1.5 }}>{item}</li>
                  ))}
                </ul>
                <div style={{ marginTop: 16, textAlign: "center" }}>
                  <button onClick={() => onEnter()} style={{ padding: "11px 20px", borderRadius: 10, background: "linear-gradient(135deg,#e8a034,#f0b44a)", border: "none", color: "#000", fontWeight: 700, fontSize: 13, cursor: "pointer", width: "100%" }}>
                    Launch AI Newsroom →
                  </button>
                </div>
              </div>

              <div style={{ background: "#0d0d14", border: "1px solid #1e1e2a", borderRadius: 14, padding: 16 }}>
                <div style={{ fontSize: 11, color: "#e8a034", marginBottom: 12, fontWeight: 600, letterSpacing: ".06em" }}>TRENDING TOPICS</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {topArticles.slice(0, 5).map((t, i) => (
                    /* [CHANGE 1] Trending topic click also opens with that article */
                    <div
                      key={i}
                      onClick={() => onEnter(t)}
                      style={{ color: "#f0ede8", fontSize: 14, lineHeight: 1.45, cursor: "pointer", paddingBottom: i < 4 ? 10 : 0, borderBottom: i < 4 ? "1px solid #1e1e2a" : "none", transition: "color .15s" }}
                      onMouseEnter={(e) => { e.currentTarget.style.color = "#e8a034"; }}
      ...
      <footer style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: "#e8a034" }}>ET</span>
...
// [CHANGE 1] AppRoot now holds selectedArticle state and passes it down.
// onEnter(article?) — article is optional. If passed, it becomes the selectedArticle.
function AppRoot() {
  const [showLanding, setShowLanding] = useState(true);
  const [selectedArticle, setSelectedArticle] = useState(null);

  const handleEnter = (article = null) => {
  ...
  return <AppInner selectedArticle={selectedArticle} onClearSelected={() => setSelectedArticle(null)} />;
}

// [CHANGE 1] AppInner receives selectedArticle and shows it pre-selected in Navigator.
function AppInner({ selectedArticle, onClearSelected }) {
  ...
  // [CHANGE 1] If a selectedArticle arrives, pre-populate the query and articles list.
  useEffect(() => {
    if (selectedArticle) {
      // Use source name or first 4 words of title as the query hint
      const autoQuery = selectedArticle.source?.name || selectedArticle.title?.split(" ").slice(0, 4).join(" ") || "";
      setQuery(autoQuery);
      // Seed articles with the selected article so tabs aren't empty
      setArticles((prev) => {
        const alreadyIn = prev.some((a) => a.url === selectedArticle.url);
        return alreadyIn ? prev : [selectedArticle, ...prev];
      });
      setTab("navigator");
    }
  }, [selectedArticle]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: DS.bg }}>
      <header style={{ borderBottom: `1px solid ${DS.border}`, padding: "0 32px", position: "sticky", top: 0, background: DS.bg, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 20, padding: "14px 0 0" }}>
            ...
            {/* [CHANGE 1] Show a "selected" badge when an article was passed from landing */}
            {selectedArticle && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: DS.accent + "15", border: `1px solid ${DS.accent}40`, borderRadius: 20, padding: "3px 12px 3px 8px", fontSize: 11 }}>
                <span style={{ color: DS.accent, fontWeight: 600 }}>◆ Article selected</span>
                <button
                  onClick={onClearSelected}
                  title="Clear selection"
                  style={{ background: "none", border: "none", color: DS.textDim, fontSize: 13, lineHeight: 1, cursor: "pointer", padding: 0 }}
                >×</button>
              </div>
            )}
      ...
      {!articles.length && !selectedArticle && tab !== "bookmarks" && (
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 32px 64px" }}>
        ...
        {/* [CHANGE 1] Pass selectedArticle into NavigatorTab */}
        {(articles.length > 0 || selectedArticle) && (
          <>
            {tab === "navigator"  && <NavigatorTab  articles={articles} bookmarks={bookmarks} selectedArticle={selectedArticle} />}
